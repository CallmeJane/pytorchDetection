# encoding=utf8
#https://www.bilibili.com/video/BV1cE411o7X8/?p=5(softmax详解)
#引用自https://blog.csdn.net/wds2006sdo/article/details/53699778
#https://github.com/WenDesi/lihang_book_algorithm/tree/master/（李航博士中所有代码实现）
import math
import pandas as pd #基于numpy的结构
import numpy as np
import random
import time
...
class Softmax(object):
    ...
    def train(self, features, labels):
        self.k = len(set(labels))                   #k分类问题

        self.w = np.zeros((self.k,len(features[0])+1))#theta，有偏置项，所以要加1
        time = 0
        ...
        while time < self.max_iteration:
            ...
            # 每次选择一个样本（或者标签）进行计算，k的个数时有限的，不是所有的赝本都能参加运算
            x = features[index]
            y = labels[index]

            x = list(x)
            x.append(1.0)              #y=w*(x,1)=(k*(n+1))*((n+1)*n)
            x = np.array(x)

            derivatives = [self.cal_partial_derivative(x,y,j) for j in range(self.k)]    #k分类问题
